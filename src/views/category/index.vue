...
<script>
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            next(vm => {
                let id = to.params.id;
                let category = vm.$store.getters.categories.find((item) => item.uri == id);
                vm.title = category.title;
                document.title = `${process.env.VUE_APP_TITLE} - ${vm.title}`;
            })
        },
        ...
        computed: {
            shelf() {
                ...
                let category = store.getters.categories.find((item) => item.uri == this.$route.path.split('/category/')[1]);
